...
class ArxivMCPTester:
    ...
    async def test_fetch_content(self, arxiv_id):
        ...
        result = await self.call_tool(
            "fetch_arxiv_paper_content", {"arxiv_id": arxiv_id, "include_pdf": False}
        )
    ...
    async def test_extract_citations(self, text):
        ...
        result = await self.call_tool("extract_citations", {"text": sample_text})
    ...
    async def test_download_convert(self, arxiv_id):
        ...
        if result["status"] == "success":
            print("✅ Download and convert successful")
            return True
    ...
    async def test_get_output_structure(self):
        ...
        result = await self.call_tool("get_output_structure", {"output_dir": "./test_output"})

        if result["status"] == "success":
            print("✅ Output structure retrieved")
        else:
    ...
    async def test_processing_metrics(self):
        ...
        result = await self.call_tool("get_processing_metrics", {"time_range": "24h"})

        if result["status"] == "success":
            print("✅ Processing metrics retrieved")
        else:
        ...
        try:
            ...
            if papers:
                ...
                if arxiv_id:
                    ...
                    # 3. Extract citations
                    if content:
                        await self.test_extract_citations(content)
