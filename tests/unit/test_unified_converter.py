...
\\end{document}"""
        ...
        with tarfile.open(fileobj=tar_buffer, mode='w') as tar:
            ...
            bib_content = b'@article{test2024, title={Test Reference}, author={Test Author}, year={2024}}'
...
\\end{document}""",
        ...
        with patch('arxiv_mcp.clients.AsyncArxivDownloader.download', new_callable=AsyncMock) as mock_download, \
             patch('arxiv_mcp.processors.LaTeXProcessor.extract_archive', return_value=mock_extracted_files), \
             patch('arxiv_mcp.processors.LaTeXProcessor.compile_latex', side_effect=Exception("PDF compilation disabled for test")), \
             patch('arxiv_mcp.utils.file_saver.FileSaver.save_latex_files', return_value={"path": f"/test/latex/{arxiv_id}", "saved": True}), \
             patch('arxiv_mcp.utils.file_saver.FileSaver.save_markdown_file', return_value=f"/test/markdown/{arxiv_id}.md"), \
             patch('arxiv_mcp.utils.file_saver.FileSaver.save_metadata', return_value=f"/test/metadata/{arxiv_id}.json"):
            ...
            assert result["summary"]["main_tex_file"] == "main.tex"
            assert result["summary"]["total_files"] == 3
            assert result["summary"]["extracted_text_length"] > 0  # Just check it's a positive number
